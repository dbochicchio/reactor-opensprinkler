...
module.exports = class OpenSprinklerController extends Controller {
	...
	async start() {
		...
		if (!this.config.topic) {
			return Promise.reject("No topic configured");
		}
		...
		if (this.mqttController === undefined) {
			try {
				...
				if (this.mqttController !== undefined) {
					...
					this.mqttController.extSubscribeTopic(this, `${this.config.topic}/#`, this.onMqttMessage.bind(this));
	...
	onMqttMessage(topic, value) {
		...
		// [topic]/system: this topic receives {"state":"started"} message when the controller boots.
		// [topic]/station/x: where x is the index (starting from 0) of the station/zone. For example, the first zone is 0, the second zone is 1 and so on.
		...
		//   where ss is the number of seconds that it ran. To receive data for all stations, you can subscribe to wildcard topic [topic]/station/#
		// [topic]/sensor1: this topic receives {"state":1} when sensor 1 activates and {"state":0} when sensor 1 deactivates.
		// [topic]/sensor2: similar to above but for sensor 2.
		// [topic]/raindelay: similar to above but for rain delay.
		// [topic]/sensor/flow: this topic receives {"count":cc,"volume":vv} when the flow sensor generates data (usually when a zone finishes running), where cc 
		//   is the flow count, and vv is the amount of volume.
		// [topic]/weather: {"water level":pp%} when the watering percentage (e.g. from automatic weather adjustment method) has changed.
		...
		const lastTopicPart = parts[parts.length - 1];

		if (lastTopicPart == "availability") {
		...
		else if (topic.includes("station/")) {
			const parts = topic.split('/');
			const stationIndex = parts.indexOf("station");

			if (stationIndex !== -1 && parts.length > stationIndex + 1) {
				const id = parseInt(parts[stationIndex + 1], 10);
				const json = JSON.parse(value);
				const state = String(json?.state) === "1";
				const duration = json?.duration ?? 0;

				this.log.notice("%1 [onMqttMessage] station: %2, %3, %4", this, id, state, duration);

				canonicalId = `os_station_${id + 1}`;
				...
				attributes = {
					...
					"toggle.state": state,
					"irrigation_zone.duration": duration,
					"irrigation_zone.remaining": state ? duration : 0
				};

				if (!state) {
					attributes["irrigation_zone.last_run"] = Math.floor(Date.now() / 1000) - duration;
				}
			}
		}
		else if (
			lastTopicPart == "raindelay" ||
			lastTopicPart == "sensor1" ||
			lastTopicPart == "sensor2"
		) {
			canonicalId = 
				lastTopicPart == "sensor1" ? "os_sensor1" :
				lastTopicPart == "sensor2" ? "os_sensor2" :
				"os_raindelay";

			var json = JSON.parse(value);
			var state = json?.state == "1";

			this.log.notice("%1 [onMqttMessage] sensor: %2, %3", this, canonicalId, state);
			...
			if (lastTopicPart == "raindelay") {
				// mqtt message does not contain duration: a refresh is needed
				needsRefresh = true;
			}
		}
		else if (lastTopicPart == "weather") {
			this.log.notice("%1 [onMqttMessage] weather: %2", this, value);

			// {"water level":pp%}
			var json = JSON.parse(value);
			var waterLevel = json?.["water level"];
			if (waterLevel !== undefined) {
				canonicalId = "os_waterlevel";
				attributes = {
					"value_sensor.value": waterLevel,
					"value_sensor.units": "%",
				};
			}
			else {
				this.log.err("%1 [onMqttMessage] weather: no water level found in %2", this, value);
			}
		}
		else {
		...
		if (!canonicalId) {
			this.log.err("%1 [onMqttMessage] error: no canonical ID found for topic %2", this, topic);
			return;
		}

		let e = this.findEntity(canonicalId);
		if (!e) {
			this.log.err("%1 [onMqttMessage] error: no entity found for %2 on topic %3", this, canonicalId, topic);
			return;
		}

		this.log.debug(5, "%1 [onMqttMessage] entity: %2 - attributes: %3", this, canonicalId, attributes);
